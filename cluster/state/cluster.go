...
func NewCluster(scheduler *scheduler.Scheduler, TLSConfig *tls.Config, discovery discovery.Backend, options cluster.DriverOpts, engineOptions *cluster.EngineOpts) (cluster.Cluster, error) {
	log.WithFields(log.Fields{"name": "state"}).Debug("Initializing cluster")
	...
	if val, ok := options.Float("state.overcommit", ""); ok {
		if val <= float64(-1) {
			log.Fatalf("state.overcommit should be larger than -1, %f is invalid", val)
		} else if val < float64(0) {
			log.Warn("-1 < state.overcommit < 0 will make swarm take less resource than docker engine offers")
	...
	if val, ok := options.Int("state.createretry", ""); ok {
		if val < 0 {
			log.Fatalf("state.createretry can not be negative, %d is invalid", val)
		}
...
func (c *Cluster) CreateContainer(config *cluster.ContainerConfig, name string, authConfig *types.AuthConfig) (*cluster.Container, error) {
	...
	if err != nil {
		...
		for ; retries < c.createRetry && err != nil; retries++ {
			log.WithFields(log.Fields{"Name": "State"}).Warnf("Failed to create container: %s, retrying", err)
			container, err = c.createContainer(config, name, false, authConfig)
		}
	}
